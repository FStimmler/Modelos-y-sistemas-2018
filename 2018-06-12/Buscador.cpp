...
void buscar(Alumno *_alumno, int cont);
...
void buscar(Alumno *_alumno, int cont){
	...
	for(int i=0;i<cont;i++){
		//b=*_alumno[i]._persona.nombre;
		if(strcmp(a,_alumno[i]._persona.nombre)==0){
			cout<<endl<<_alumno[i]._persona.nombre<<" Encontrado en la posicion numero "<<i+1;
			return;
		}
